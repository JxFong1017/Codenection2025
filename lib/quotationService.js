...
async function generateQuotationAndSendEmail(quotationData) {
    ...
    if (selectedProtections.length > 0) {
     ...
     const quotationToSave = {
        user_email: email, // CORRECTED: Changed 'userId' to 'user_email' to match dashboard query
        quotation_no,
        date,
        ...
        coverage_type: coverage.type, // Ensure field name consistency
        status: 'pending',
        additional_protections_list: protectionsList,
        ...prices,
        createdAt: new Date(), // ADDED: Add the timestamp for sorting
      };
