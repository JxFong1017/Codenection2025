...
async function generateQuotationAndSendEmail(quotationData) {
    ...
    try {
        console.log('Attempting to save quotation to Firestore...'); // ADD THIS LOG
        const docRef = await db.collection('quotations').add(quotationToSave);
        console.log('Quotation saved with ID: ', docRef.id); // ADD THIS LOG
        } catch (error) {
        console.error('Error saving quotation to Firestore: ', error); // THIS IS CRITICAL
        throw new Error('Failed to save quotation to the database.');
        }
        
        console.log('Preparing email content...'); // ADD THIS LOG
